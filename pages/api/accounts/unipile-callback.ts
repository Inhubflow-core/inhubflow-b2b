...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(200).json({ received: true, accountId: state.accountId, unipileAccountId: remoteAccountId, status: remoteStatus });
  } catch (error) {
    return res.status(502).json({ error: error instanceof Error ? error.message : "No se pudo verificar la cuenta Unipile" });
  }
}
